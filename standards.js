...
const ACHIEVEMENT_STANDARDS = Object.fromEntries(
  Object.entries({ ...RAW_STANDARDS, ...SUPPLEMENTAL_RAW_STANDARDS, ...MANUAL_RAW_STANDARDS, ...INFORMATION_RAW_STANDARDS }).map(([subject, text]) => [subject, text.trim().split('\n').map(line => line.trim()).filter(Boolean).map(line => {
    const match = line.match(/^\[([^\]]+)]\s*(.*)$/);
    return { code: match[1], text: match[2] };
  })])
);
// 교과 공통 기능과 과목별 자료를 분리하기 위한 첫 번째 교과 프로필입니다.
// 이후 다른 교과 자료를 받으면 같은 형식으로 대분류와 소분류를 추가할 수 있습니다.
const SUBJECT_PROFILES = {
  math: {
    ...
    courses: [
      ...
      { value: '미적분Ⅱ', label: '미적분Ⅱ' },
      { value: '확률과 통계', label: '확률과 통계' },
      { value: '기하', label: '기하' },
      { value: '경제수학', label: '경제 수학' },
      { value: '인공지능수학', label: '인공지능 수학' }
    ]
  }
};
